...
print("===== 186_LangChain_Wikipedia_English_Query_Agent實驗 =====\n")

# Wikipedia Tool（維基百科工具）
@tool
def wikipedia_search(query: str) -> str:
    """
    當你需要查詢地理或名詞解釋、程式語言、科學概念，
    請將中文轉換為英文，再送入 Wikipedia 工具查詢
    """
...
prompt_template = ChatPromptTemplate.from_messages([
    (
        ...
        1. 回答必須使用繁體中文
        2. 回答需符合台灣用語
        3. 使用 Wikipedia 工具前，請先將中文名詞轉為英文
        """
    ),
    ("human", "{input}")
])
...
def run_custom_agent(user_input: str):
    ...
    if ai_msg.tool_calls:
        ...
        for tool_call in ai_msg.tool_calls:
            ...
            print("=" * 50)
            print(f"[系統偵測] 模型決定調用工具：{tool_name}")
            print()
            ...
            print(f"[系統執行] 工具返回結果：{observation}")
            print("=" * 50)
...
# Query（查詢問題）
response = run_custom_agent("Pongso no Tao是甚麼地方？")
